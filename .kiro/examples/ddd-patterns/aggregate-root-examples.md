...
**Aggregate Root（聚合根）** 是聚合的入口點 - 一組可以作為單一單元處理的領域物件的集合。只有聚合根可以從 repositories 取得，且所有對聚合的變更都必須透過根進行。

## 核心概念

- **一致性邊界（Consistency Boundary）**：Aggregate 定義交易邊界
- **事件收集（Event Collection）**：Aggregate 收集領域事件
- **不變條件保護（Invariant Protection）**：Aggregate 強制執行業務規則
- **單一入口點（Single Entry Point）**：只有根可以從外部存取

---

## 範例 1：Order Aggregate（正式程式碼）

### 完整實作

這是我們正式程式碼庫中實際的 Order aggregate，展示了真實世界的 DDD patterns：
...
## 範例 2：Customer Aggregate（正式程式碼）

這是我們正式程式碼庫中實際的 Customer aggregate：
...
### 3. Cross-Aggregate Operations（跨聚合操作）
